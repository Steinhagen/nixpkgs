...
if (defined $ENV{WINEPREFIX} and -d "$ENV{WINEPREFIX}/desktopdata/cxmenu")
    ...
    # The Windows-side demo check in winewrapper.exe (32- and 64-bit) runs
    # against "$CX_ROOT/etc/license.txt" in the read-only store, where a
    # registered license can never appear. The launchers already put the
    # user cellar on the wine environment as CX_HOME, so retarget the check
    # (and the tie.pub verification-key lookup, which reads the same env
    # string) to "$CX_HOME/etc": the same directory the patched GUI writes
    # registration files to, and where a declarative license lands.
    #
    # The byte pattern "CX_ROOT\0\0\0" is the padded string table entry
    # shared by the license check and the key load; the other "CX_ROOT\0"
    # entry (wine's install-root helper) is left untouched.
    for winewrapper in \
      $out/lib/wine/x86_64-windows/winewrapper.exe \
      $out/lib/wine/i386-windows/winewrapper.exe
    do
      before=$(grep -abo "CX_ROOT" "$winewrapper" | wc -l)
      if [ "$before" != 2 ]; then
        echo "crossover: unexpected CX_ROOT string layout in $winewrapper" >&2
        exit 1
      fi
      perl -0777 -pi -e 's/CX_ROOT\x00\x00\x00/CX_HOME\x00\x00\x00/g' "$winewrapper"
      after=$(grep -abo "CX_ROOT" "$winewrapper" | wc -l)
      if [ "$after" != 1 ] || ! grep -aq "CX_HOME" "$winewrapper"; then
        echo "crossover: failed to retarget license check in $winewrapper" >&2
        exit 1
      fi
    done

    # winewrapper.exe also loads its signature-verification key from
    # "$CX_HOME/share/crossover/data/tie.pub"; the key ships in the store,
    # so the launchers bridge it into the cellar on first run. The wine
    # launcher is the single real file: autorun, cxstart, regedit and
    # regsvr32 are symlinks to it.
    substituteInPlace $out/bin/wine \
      --replace-fail '    $ENV{CX_HOME}=CXBottle::get_user_dir();' \
                     '    $ENV{CX_HOME}=CXBottle::get_user_dir();
    my $cx_tie_pub="$ENV{CX_HOME}/share/crossover/data/tie.pub";
    if (!-e $cx_tie_pub and -f "$ENV{CX_ROOT}/share/crossover/data/tie.pub")
    {
        require File::Path;
        File::Path::make_path("$ENV{CX_HOME}/share/crossover/data");
        symlink("$ENV{CX_ROOT}/share/crossover/data/tie.pub", $cx_tie_pub);
    }'

    # The perl-side registration probe used by cxdiag and friends reads the
    # store path as well; point it at the cellar like everything else.
    substituteInPlace $out/lib/perl/CXUtils.pm \
      --replace-fail 'sub license_file_present()
{
                     ...
                     'sub license_file_present()
{
    ...
    return (defined $user_dir and -f "$user_dir/etc/license.txt");
}'
